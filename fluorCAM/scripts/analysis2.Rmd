...
plot1 <- data_all %>% ggplot(aes(x=plant, y=Rfd_Lss, fill=inoculation)) +
  ...
  labs(x = "",
       title = "Rfd_Lss w zależności od grupy roślin, pomiar 2") +
...
plot3 <- data_all %>% ggplot(aes(x=plant, y=NPQ_Lss, fill=inoculation)) +
  ...
  scale_fill_manual(values = c("#66CCCC", "#006666"), name = "") +
  labs(x = "",
       title = "NPQ_Lss w zależności od grupy roślin, pomiar 2") +
  geom_signif(comparisons = list(c("AT5G45470","WT")), 
              map_signif_level = TRUE, test = "t.test", step_increase = 0.2) +
...
```{r}
ggsave(filename = '/home/konsternacja/Git/master-thesis1/figures/plots/rfd_lss_pomiar2.png',plot = plot1, width = 12, height = 8)
ggsave(filename = '/home/konsternacja/Git/master-thesis1/figures/plots/npq_lss_pomiar2.png',plot = plot3, width = 12, height = 8)
```
